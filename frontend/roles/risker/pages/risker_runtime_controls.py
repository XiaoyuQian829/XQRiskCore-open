# frontend/roles/risker/pages/runtime_controls.py
...
def render(ctx: RequestContext, client: ClientContext):
    if not ctx.has_permission("risker.trigger_global_killswitch"):
        st.warning("You do not have permission to access runtime controls.")
        return
    ...
    risker_user = ctx.user_id or "risker"
    ...
    with col1:
        ...
        if not silent_active:
            if st.button("Trigger Account Silent (5 days)"):
                ks.trigger_silent_all(days=5, reason="Manual trigger from risker", user_id=risker_user)
                client.save()
            ...
            if st.button("Release Account Silent"):
                ks.release_silent_all(user_id=risker_user)
                client.save()
    ...
    with col2:
        ...
        if not killswitch_active:
            if st.button("Trigger Account Kill Switch"):
                ks.trigger_killswitch_all(reason="Manual kill", user_id=risker_user)
                client.save()
            ...
            if st.button("Release Account Kill Switch"):
                ks.release_killswitch_all(user_id=risker_user)
                client.save()
    ...
    with col3:
        ...
        if not silent_active:
            if st.button("Trigger Silent for Asset"):
                ks.trigger_silent(selected_asset, days=silent_days, reason="Manual silent", user_id=risker_user)
                client.save()
            ...
            if st.button("Release Silent for Asset"):
                ks.release_silent(selected_asset, user_id=risker_user)
                client.save()
    ...
    with col4:
        ...
        if not killswitch_active:
            if st.button("Trigger Kill Switch for Asset"):
                ks.trigger_killswitch(selected_asset, reason="Manual kill", user_id=risker_user)
                client.save()
            ...
            if st.button("Release Kill Switch for Asset"):
                ks.release_killswitch(selected_asset, user_id=risker_user)
                client.save()
